...
export class DexMempoolLoop implements DexLoopInterface {
	...
	public constructor(
		...
		for (const op of filteredPools) {
			console.log(op.address, op.pairType, op.dexname);
		}
		for (const path of paths) {
			if (path.pools.some((pool) => pool.dexname === AmmDexName.default)) {
				for (const p of path.pools) {
					console.log(p.address, p.dexname, p.pairType);
				}
